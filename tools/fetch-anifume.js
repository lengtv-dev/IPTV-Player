 ...
 *
 * ─── Flags ───────────────────────────────────────────────────────────────
 *   <url>              URL หน้ารายการบน anifume.com (จำเป็น ยกเว้นใช้ --update-meta)
 ...
 *
 *   # Fetch ซับไทย
 *   node fetch-anifume.js https://anifume.com/41410 --track=subth --output=FILENAME.txt
 *
 *   # Merge พากย์ไทยเข้าไฟล์เดิม
 *   node fetch-anifume.js https://anifume.com/41411 --track=th --output=FILENAME.txt
 *
 *   # Fetch season 2
 *   node fetch-anifume.js https://anifume.com/41500 --track=subth --season=2 --output=FILENAME.txt
 *
 *   # อัปเดต metadata
 *   node fetch-anifume.js --update-meta --output=FILENAME.txt
 *
 * ─── หมายเหตุ ────────────────────────────────────────────────────────────
 *   - Stream URL มี token หมดอายุ (e= Unix timestamp) ต้อง re-fetch เมื่อหมดอายุ
 *   - Video URL รูปแบบ: https://a13.rukoluo.com/f/{series}/{ep}.mp4?m=...&e=...
 *   - ใช้ 720p เป็น default, fallback 360p
...
if (!seriesUrl && !updateMeta) {
  ...
  console.error("       node fetch-anifume.js --update-meta[=poster|cover|title] [--season=N] [--track=th|subth] --output=FILE.txt");
  process.exit(1);
}
...
async function parseSeriesPage(url) {
  ...
  // Series ID from URL
  const seriesId = url.match(/anifume\.com\/(\d+)/)?.[1] || "";

  // Title — try multiple selectors
  const title = $("h1").first().text().trim()
    || $(".entry-title, .title, h2").first().text().trim()
    || $("title").text().replace(/\s*[-|].*$/, "").trim();

  // Poster image
  const posterImg = $(`img[src*="anifume.com/images"]`).first().attr("src")
    || $(".poster img, .thumb img").first().attr("src")
    || $("article img, .content img").first().attr("src")
    || "";

  // Episode links — filter links that contain /{seriesId}/
  const seen = new Set();
  const episodes = [];

  $(`a[href*="/${seriesId}/"]`).each((_, el) => {
    const href = $(el).attr("href");
    if (!href || seen.has(href)) return;
    seen.add(href);

    const epUrl = href.startsWith("http") ? href : `https://anifume.com${href}`;
    const thumb = $(el).find("img").attr("src") || "";
    const epTitle = $(el).text().trim();
    episodes.push({ url: epUrl, thumb, epTitle });
  });

  // Sort by episode number at end of URL (e.g. -01, -02)
  episodes.sort((a, b) => {
    const numA = parseInt((a.url.match(/-(\d+)$/) || [])[1] || 0);
    const numB = parseInt((b.url.match(/-(\d+)$/) || [])[1] || 0);
    return numA - numB;
  });
...
// Extract mainv() player endpoint URL from episode page HTML
function extractPlayerUrl(html) {
  // mainv() function contains the primary player iframe src
  const mainvMatch = html.match(/function\s+mainv\s*\(\s*\)[^{]*\{[^}]*src="(https:\/\/anifume\.com\/player\/[^"]+)"/);
  if (mainvMatch) return mainvMatch[1];

  // Fallback: any iframe pointing to /player/
  const iframeMatch = html.match(/src="(https:\/\/anifume\.com\/player\/[^"]+)"/);
  if (iframeMatch) return iframeMatch[1];

  return null;
}

// Extract video URL from JW Player config on player endpoint page
function extractVideoUrl(playerHtml) {
  // Match all "file": "https://..." entries
  const fileMatches = [...playerHtml.matchAll(/"file"\s*:\s*"(https?:\/\/[^"]+\.mp4[^"]+)"/g)];
  if (fileMatches.length === 0) return null;

  // Prefer 720p (no -360 suffix), fallback to first found
  const hq = fileMatches.find((m) => !m[1].includes("-360.mp4"));
  return (hq || fileMatches[0])[1];
}

// Log token expiry for user awareness
function logExpiry(videoUrl) {
  const eMatch = videoUrl?.match(/[?&]e=(\d+)/);
  if (eMatch) {
    const exp = formatExpiry(parseInt(eMatch[1]));
    console.log(`     ⏱  token หมดอายุ: ${exp}`);
  }
}
...
async function getStreamUrl(epPageUrl) {
  ...
  // 1. Extract player endpoint URL from mainv()
  const playerUrl = extractPlayerUrl(html);
  if (!playerUrl) throw new Error(`ไม่พบ player URL ใน ${epPageUrl}`);

  // 2. Fetch player endpoint (needs Referer: anifume.com)
  const playerHtml = await fetchHtml(playerUrl, { Referer: REFERER });

  // 3. Extract video URL from JW Player config
  const videoUrl = extractVideoUrl(playerHtml);
  if (!videoUrl) throw new Error(`ไม่พบ video URL จาก player endpoint`);
...
async function main() {
  ...
  try {
    ...
    console.log(`   ⚠️  หมายเหตุ: stream URL มี token หมดอายุ ต้อง re-fetch เมื่อใช้ไม่ได้`);
  } catch (err) {
    console.error(`\n❌ Error: ${err.message}`);
    process.exit(1);
  }
