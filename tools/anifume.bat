...
echo.
set "URL="
set /p URL=anifume URL (e.g. https://anifume.com/41410):
if "%URL%"=="" goto FETCH
...
echo.
set "CMD=node fetch-anifume.js %URL% --track=%TRACK% --season=%SEASON% --output=%OUTPUT%"
if not "%TMDB_ID%"=="" set "CMD=%CMD% --tmdb-id=%TMDB_ID%"

echo  ^ %CMD%
echo.
echo  NOTE: stream URLs have expiry tokens - re-fetch when expired
echo.
%CMD%
goto DONE
