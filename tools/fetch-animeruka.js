 ...
 *
 * ─── Flags ───────────────────────────────────────────────────────────────
 *   <url>              URL หน้ารายการบน animeruka.com (จำเป็น ยกเว้นใช้ --update-meta)
 *                      รูปแบบ: https://animeruka.com/anime/SLUG/
 ...
 *
 *   # Fetch ซับไทย
 *   node fetch-animeruka.js https://animeruka.com/anime/SLUG/ --track=subth --output=FILENAME.txt
 *
 *   # Merge พากย์ไทยเข้าไฟล์เดิม
 *   node fetch-animeruka.js https://animeruka.com/anime/SLUG/ --track=th --output=FILENAME.txt
 *
 *   # Fetch season 2
 *   node fetch-animeruka.js https://animeruka.com/anime/SLUG/ --track=subth --season=2 --output=FILENAME.txt
 *
 *   # อัปเดต metadata
 *   node fetch-animeruka.js --update-meta --output=FILENAME.txt
 *
 * ─── หมายเหตุ ────────────────────────────────────────────────────────────
 *   - Stream URL รูปแบบ: https://cdn2.maimeorder.com/hls/{TOKEN}.txt
 *   - ใช้ DooPlay + animemami.xyz chain
...
if (!seriesUrl && !updateMeta) {
  ...
  console.error("       node fetch-animeruka.js --update-meta[=poster|cover|title] [--season=N] [--track=th|subth] --output=FILE.txt");
  process.exit(1);
}
...
const REFERER_ANIMERUKA = "https://animeruka.com/";
const REFERER_ANIMEMAMI = "https://animemami.xyz/";
...
async function parseSeriesPage(url) {
  ...
  const html = await fetchHtml(url, { Referer: REFERER_ANIMERUKA });
  const $ = cheerio.load(html);

  // Series title — try multiple selectors
  const title = $("h1").first().text().trim()
    || $(".entry-title, .sheader .title, .data h1").first().text().trim()
    || $("title").text().replace(/\s*[-|].*$/, "").trim();

  // Poster image
  const posterImg = $(".poster img, .sheader .poster img").first().attr("src")
    || $(`img[src*="tmdb"], img[src*="poster"]`).first().attr("src")
    || $("article img").first().attr("src")
    || "";

  // Collect all episode links — /ep/ URLs
  const seen = new Set();
  const allEpLinks = [];
  ...
  $("a[href*='/ep/']").each((_, el) => {
    ...
    seen.add(href);
    const epUrl = href.startsWith("http") ? href : `https://animeruka.com${href}`;
    allEpLinks.push(epUrl);
  });

  // Apply --track filter
  // th (dubbed): keep URLs containing "thai"
  // subth (subbed): keep URLs NOT containing "thai"
  let episodes;
  if (trackArg === "th") {
    episodes = allEpLinks.filter((u) => u.toLowerCase().includes("thai"));
  } else if (trackArg === "subth") {
    episodes = allEpLinks.filter((u) => !u.toLowerCase().includes("thai"));
  } else {
    episodes = allEpLinks;
  }

  // Sort by episode number (numeric part before -thai or end of path segment)
  episodes.sort((a, b) => {
  ...
  console.log(`✅ พบ: "${title}" — ${episodes.length} ตอน (จากทั้งหมด ${allEpLinks.length} links)`);
  return { title, posterImg, episodes };
}

function extractEpNumber(url) {
  // e.g. /ep/slug-ep-12/ or /ep/slug-ep-12-thai/
  const m = url.match(/ep-(\d+)(?:-thai)?(?:\/|$)/i)
    || url.match(/-(\d+)(?:-thai)?(?:\/|$)/i)
    || url.match(/\/(\d+)(?:\/|$)/);
  return m ? parseInt(m[1]) : 0;
}

// ───── Step 2: Extract POST_ID from episode page ─────
async function extractPostId(epPageUrl) {
  const html = await fetchHtml(epPageUrl, { Referer: REFERER_ANIMERUKA });
  const $ = cheerio.load(html);

  // Try data-post or data-postid attributes first
  const dataPost = $("[data-post]").first().attr("data-post")
    || $("[data-postid]").first().attr("data-postid");
  if (dataPost) return dataPost;

  // Try inline scripts for "post":"N" or "post_id":"N" patterns
  let postId = null;
  $("script").each((_, el) => {
    if (postId) return;
    const src = $(el).html() || "";
    const m = src.match(/"post"\s*:\s*"?(\d+)"?/)
      || src.match(/"post_id"\s*:\s*"?(\d+)"?/)
      || src.match(/['"](post|postid)['"]\s*:\s*"?(\d+)"?/);
    if (m) postId = m[m.length - 1];
  });
  if (postId) return postId;

  // Fallback: look in page HTML for data-post="N"
  const rawMatch = html.match(/data-post(?:id)?="(\d+)"/);
  if (rawMatch) return rawMatch[1];

  throw new Error(`ไม่พบ POST_ID ใน ${epPageUrl}`);
}

// ───── Step 3: Fetch DooPlay embed URL ─────
async function fetchDooplayEmbedUrl(postId) {
  const apiUrl = `https://animeruka.com/wp-json/dooplayer/v2/${postId}/tv/1`;
  let data;
  try {
    data = await fetchJson(apiUrl, { Referer: REFERER_ANIMERUKA });
  } catch (err) {
    throw new Error(`DooPlay API ล้มเหลว (post=${postId}): ${err.message}`);
  }

  // Response: [{n:1, d:{embed_url:"https://animemami.xyz/v/SLUG",...}}, ...]
  // Find entry where n == 1
  const entry = Array.isArray(data) ? data.find((e) => e.n === 1 || e.n === "1") : null;
  const embedUrl = entry?.d?.embed_url || entry?.embed_url || null;

  if (!embedUrl) {
    // Try flat object
    if (data?.embed_url) return data.embed_url;
    throw new Error(`ไม่พบ embed_url จาก DooPlay API (post=${postId})`);
  }
  return embedUrl;
}

// ───── Step 4: Fetch m3u8 URL from animemami.xyz ─────
async function fetchAnimemamiM3u8(embedUrl) {
  const html = await fetchHtml(embedUrl, { Referer: REFERER_ANIMERUKA });
  const $ = cheerio.load(html);

  // Vue/Inertia app: <div id="app" data-page="...JSON...">
  const dataPage = $("#app").attr("data-page");
  if (!dataPage) {
    throw new Error(`ไม่พบ #app[data-page] ใน ${embedUrl}`);
  }
  ...
  try {
    ...
    throw new Error(`parse data-page JSON ล้มเหลว: ${e.message}`);
  }

  // props.video.url = m3u8 URL
  const m3u8Url = pageData?.props?.video?.url
    || pageData?.props?.videoUrl
    || null;

  if (!m3u8Url) {
    throw new Error(`ไม่พบ props.video.url ใน data-page JSON (${embedUrl})`);
  }

  return m3u8Url;
}

// ───── Step 2-4 combined: Get stream URL for one episode page ─────
async function getStreamUrl(epPageUrl) {
  // Step 2: extract POST_ID
  const postId = await extractPostId(epPageUrl);

  // Step 3: fetch DooPlay embed URL
  const embedUrl = await fetchDooplayEmbedUrl(postId);

  // Step 4: fetch m3u8 from animemami.xyz
  const m3u8Url = await fetchAnimemamiM3u8(embedUrl);

  return m3u8Url;
}

// ───── Episode title from page ─────
async function getEpTitle(epPageUrl, seriesTitle) {
  try {
    const html = await fetchHtml(epPageUrl, { Referer: REFERER_ANIMERUKA });
    const $ = cheerio.load(html);
    ...
    // Remove series title prefix (clean up "SeriesName - Episode X - EpTitle" → "EpTitle")
    let cleaned = raw;
    if (seriesTitle) {
      const escapedSeries = seriesTitle.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
      cleaned = cleaned.replace(new RegExp(`^${escapedSeries}\\s*[-:–]?\\s*`, "i"), "");
    }
    // Remove leading "Episode N" / "Ep N" / "ตอนที่ N" prefix if still present
    cleaned = cleaned.replace(/^(?:Episode|Ep\.?|ตอนที่|ตอน)\s*\d+\s*[-:–]?\s*/i, "").trim();
...
// ───── TMDB metadata helpers (copied from fetch-anifume.js) ─────
...
async function main() {
  ...
  try {
    ...
    if (episodes.length === 0) {
      console.error("❌ ไม่พบ episode ใดเลย ตรวจสอบ URL หรือ --track flag อีกครั้ง");
      process.exit(1);
    }
    ...
    for (let i = 0; i < episodes.length; i++) {
      ...
      let streamUrl = null;
      try {
        streamUrl = await getStreamUrl(epUrl);
        process.stdout.write(` ✅\n`);
      } catch (err) {
        console.warn(` ⚠️  ${err.message} — ใช้ episode page URL เป็น fallback`);
      }
    ...
    console.log(`   หมายเหตุ: stream URL รูปแบบ HLS จาก cdn2.maimeorder.com`);
  } catch (err) {
    console.error(`\n❌ Error: ${err.message}`);
    process.exit(1);
  }
